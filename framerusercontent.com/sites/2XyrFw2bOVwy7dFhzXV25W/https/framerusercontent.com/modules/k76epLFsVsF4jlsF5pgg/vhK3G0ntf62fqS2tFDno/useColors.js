...
export function useColors(...colors) {
    ...
    for (let i = 0; i < colors.length; i++) {
        const color = colors[i]; // Return original value if not a string
...
function extractStyleValues(variableNames) {
    ...
    const allVarNames = new Set([...Object.keys(lightVars), ...Object.keys(darkVars)]);
...
function extractDefaultValue(cssVar) { // Check if the string starts with "var("
